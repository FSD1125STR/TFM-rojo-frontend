...
async function updateTeamRecords(teamSeleccionado, datos) {
...
async function createTeamRecords(datos) {
...
export function TeamsList() {
  ...
  const loadTeams = () => {
    ...
    getAdminTeams()
      .then(setTeams)
      .catch((err) => showError(getApiErrorMsg(err, 'Error al cargar los equipos')))
  ...
  const handleGuardar = async (datos) => {
    ...
    try {
      if (teamSeleccionado) {
        await updateTeamRecords(teamSeleccionado, datos);
        closeLoading();
        showToast('Equipo actualizado correctamente');
      } else {
        const created = await createTeamRecords(datos);
        closeLoading();
        if (!created) return;
